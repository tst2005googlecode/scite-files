There is a Python recipe that, while not an interactive debugger, makes it easier to debug Python scripts within SciTE. It was found [here](http://code.activestate.com/recipes/52215/). Thanks to Bryn Keller and Jim Eggleston.

Place the following in a file <tt>debug.py</tt>:
...
Now, in python.properties you can add the following

```
command.name.4.*.py=Simple debugger
command.4.*.py=python c:\path\to\debug.py "$(FilePath)"
```